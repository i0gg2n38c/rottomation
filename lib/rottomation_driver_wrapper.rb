...
module Rottomation
  class RottomationDriverWrapper < RottomationLogger
    ...
    def initialize(test_name:) # rubocop:disable Metrics/AbcSize
      super(test_name:)
    ...
    def run_head_on(url:, driver:) # rubocop:disable Metrics/AbcSize
    ...
    def click_element(element_name:, **kargs)
      wait_for_element(kargs).find_element(kargs, element_name:).click
    end
  end
end
